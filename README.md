...
**Перемкнути модель = змінити одне поле `model` у воркфлоу.** Точний розклад витрат — у таблиці нижче.

---

## 💰 Бюджет: точна вартість кожного сервісу

**Розрахунок на:** ~900 матеріалів/міс (30/день), ~10 тис. токенів входу + 4 тис. виходу на матеріал. Ціни в USD. Проєкт — 6 місяців (черв–лист 2026).

### Усі сервіси стека

| Сервіс | За день | За місяць | За рік | Примітка |
|---|---:|---:|---:|---|
| WordPress (хостинг + домен) | — | — | — | існуючий сайт, окремий бюджет редакції |
| WPVibe | $0 | $0 | $0 | Free-план (як на matsuka.online) |
| Yoast SEO (free) | $0 | $0 | $0 | Premium — опція: ≈ $8.25/міс = **$99/рік** |
| **n8n — VPS self-hosted** | **$0.20** | **$6** | **$72** | Hetzner/DigitalOcean; фолбек Make.com free = $0 |
| **Claude API — Opus 4.8 (дефолт)** | **$4.50** | **$135** | **$1 620** | найякісніше; дешевше — Sonnet/Haiku (нижче) |
| Telegram Bot API | $0 | $0 | $0 | безкоштовно |
| Google Sheets + Looker Studio | $0 | $0 | $0 | безкоштовно |
| Notion | $0 | $0 | $0 | free-план / вже використовується |
| Canva | $0 | $0 | $0 | наявний план редакції |
| Final Cut Pro | $0 | $0 | $0 | наявна ліцензія (одноразова) |
| Whisper (транскрибація) | $0 | $0 | $0 | працює локально на комп'ютері редакції |
| GA4 | $0 | $0 | $0 | безкоштовно |

> **Реальні нові витрати — лише два рядки:** VPS для n8n і токени Claude API. Решта — безкоштовна або вже оплачена редакцією.

### Claude API — залежно від моделі

| Модель | За день | За місяць | За рік |
|---|---:|---:|---:|
| Opus 4.8 (дефолт, максимальна якість) | $4.50 | $135 | $1 620 |
| Sonnet 5 (інтро-ціни до 31.08.2026) | $1.80 | $54 | — |
| Sonnet 5 (повна ціна) | $2.70 | $81 | $972 |
| Haiku 4.5 (економ-режим) | $0.90 | $27 | $324 |

### Разом по системі (VPS $6/міс + Claude API)

| Сценарій | За день | За місяць | За рік | **За 6 міс (проєкт)** |
|---|---:|---:|---:|---:|
| 🟢 Економ (Haiku) | ~$1.10 | **$33** | $396 | **$198** |
| 🟡 Баланс (Sonnet, повна ціна) | ~$2.90 | **$87** | $1 044 | **$522** |
| 🔴 Максимум (Opus 4.8) | ~$4.70 | **$141** | $1 692 | **$846** |

> 💡 **Гнучкість:** можна почати на Haiku ($33/міс), а критичні матеріали ганяти через Opus — гібрид дає якість там, де вона потрібна, за ціною ближче до економ-сценарію. Sonnet до 31.08.2026 працює за інтро-цінами (−33%), що додатково здешевлює перші місяці проєкту.

---

## 📗 Перевірений кейс: matsuka.online

Підхід не теоретичний — він уже працює на сайті [«Аналітична група Олексія Мацуки»](https://www.matsuka.online) (WordPress, новинна тема Newspaper). Сайт підключено до WPVibe і він керується через Claude у Cowork: контент, правки, SEO — без паролів і ручного адмінування. Звідти ж — перевірена конфігурація плагінів, яку переносимо на сайт НД:

| Перевірено на matsuka.online | Роль | Що робимо для НД |
|---|---|---|
| Підключення до WPVibe (один клік, без паролів) | AI-керування сайтом із Claude: контент, WP-CLI, масові операції | повторюємо процедуру — крок місяця 2 |
| Yoast SEO (там — Premium) | мета-теги, sitemap, редиректи | стартуємо на free; Premium — за результатами пілота |
| Integromat (Make) Connector | міст WordPress ↔ автоматизації — патерн уже в продакшні | той самий патерн через вебхуки WP ↔ n8n (або Make) |
| Code Snippets | PHP-сніпети без правки теми | ставимо сніпет Yoast-полів для REST ([інструкція](workflows/README.md)) |
| Site Kit by Google | GA4 + Search Console в адмінці | дані для M&E-плану проєкту |
| Schema & Structured Data | розмітка статей для пошуку | видимість новин у Google |
| UpdraftPlus | бекапи | обов'язковий бекап перед кожним етапом впровадження |
| Плагін безпеки + кешування | базова гігієна сайту | перевірити відповідники на сайті НД |

**Висновок для команди:** процедура «підключити WPVibe → керувати сайтом через Claude → з'єднати WordPress з автоматизаціями через вебхуки/конектор» відпрацьована на живому сайті. Для НД її повторюємо, а не винаходимо з нуля — це знімає головний ризик місяців 2–3.
...
- [ ] Підключити сайт НД до WPVibe за процедурою, відпрацьованою на matsuka.online ([кейс](#-перевірений-кейс-matsukaonline))
- [ ] Поставити перевірений набір плагінів: Yoast SEO (free), Code Snippets, UpdraftPlus, Site Kit
...
- Сайт на WordPress працює; акаунт WPVibe активний — **сайт НД ще не підключено** (крок місяця 2; процедура вже відпрацьована на matsuka.online)
